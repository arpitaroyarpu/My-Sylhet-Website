...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM users_login WHERE email='{$email}'")) > 0) {
        $query = mysqli_query($conn, "UPDATE users_login SET code='{$code}' WHERE email='{$email}'");
        ...
        if ($query) {
            ...
            try {
                ...
                $mail->Body    = 'Here is the verification link <b><a href="http://localhost/MySylhet/regform/change-password.php?reset=' . $code . '">http://localhost/MySylhet/regform/change-password.php?reset=' . $code . '</a></b>';
