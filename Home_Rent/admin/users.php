...
if (isset($_POST['delete'])) {
   ...
   $verify_delete = $conn->prepare("SELECT * FROM `users_login` WHERE id = ?");
   ...
   if ($verify_delete->rowCount() > 0) {
      ...
      $delete_user = $conn->prepare("DELETE FROM `users_login` WHERE id = ?");
   ...
   <section class="grid">
      ...
      <div class="box-container">
         ...
         if (isset($_POST['search_box']) or isset($_POST['search_btn'])) {
            ...
            $select_users = $conn->prepare("SELECT * FROM `users_login` WHERE name LIKE '%{$search_box}%' OR number LIKE '%{$search_box}%' OR email LIKE '%{$search_box}%'");
            $select_users->execute();
         } else {
            $select_users = $conn->prepare("SELECT * FROM `users_login`");
            $select_users->execute();
         }
         ...
         ?>
               <div class="box">
                  ...
                  <p>number : <a href="tel:<?= $fetch_users['mobile']; ?>"><?= $fetch_users['mobile']; ?></a></p>
