...
$select_user = $conn->prepare("SELECT * FROM `users_login` WHERE id = ? LIMIT 1");
...
if (isset($_POST['submit'])) {
   ...
   if (!empty($name)) {
      $update_name = $conn->prepare("UPDATE `users_login` SET name = ? WHERE id = ?");
      $update_name->execute([$name, $user_id]);
      $success_msg[] = 'name updated!';
   }

   if (!empty($email)) {
      $verify_email = $conn->prepare("SELECT email FROM `users_login` WHERE email = ?");
      $verify_email->execute([$email]);
      if ($verify_email->rowCount() > 0) {
         $warning_msg[] = 'email already taken!';
      } else {
         $update_email = $conn->prepare("UPDATE `users_login` SET email = ? WHERE id = ?");
   ...
   if (!empty($number)) {
      $verify_number = $conn->prepare("SELECT mobile FROM `users_login` WHERE mobile = ?");
      $verify_number->execute([$number]);
      if ($verify_number->rowCount() > 0) {
         $warning_msg[] = 'number already taken!';
      } else {
         $update_number = $conn->prepare("UPDATE `users_login` SET mobile = ? WHERE id = ?");
...
<body>
    ...
    <section class="form-container">
        ...
        <form action="" method="post">
            ...
            <input type="number" name="mobile" min="0" max="9999999999" maxlength="10"
                placeholder="<?= $fetch_user['mobile']; ?>" class="box">
